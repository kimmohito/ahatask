    ...
    useEffect(() => {
        const loadStatuses = async () => {
            ...
            try {
                ...
                if (res?.data) {
                    // assume an array of status strings or objects with `name`
                ...
                <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.max(statuses.length, 1)}, 1fr)`, gap: 12 }}>
                    ...
                    {statuses.map((status) => (
                        <div key={status} style={{ border: "1px solid #ddd", padding: 8, borderRadius: 6 }}>
                            ...
                            {/* Add item form for this status */}
                            <AddTaskByStatus status={status} org={org} project={project} onAdded={() => fetchTasks()} />
...
function AddTaskByStatus({ status, onAdded, org, project }: { status: string; org: string; project: string; onAdded?: () => void }) {
    ...
    const add = async () => {
        ...
        setSaving(true);
        try {
            await api.post("/api/tasks", { title: title.trim(), status, org, project });
            setTitle("");
            onAdded && onAdded();
        } catch (e) {
    ...
    return (
        <div style={{ marginBottom: 8 }}>
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={`Add to ${status}`} style={{ width: "100%", boxSizing: "border-box" }} />
            <button onClick={add} disabled={saving || !title.trim()} style={{ marginTop: 6 }}>Add</button>
        </div>
    );
}
